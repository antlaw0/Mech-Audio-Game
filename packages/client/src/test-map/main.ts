...
function startTestMap(): void {
  ...
  totalWeight: number,
  groundRatedLoad: number
): {
  ratedLoad: number
  loadRatio: number
  weightFactor: number
} => {
  const safeRatedLoad = Math.max(1, groundRatedLoad)
  const loadRatio = totalWeight / safeRatedLoad

  return {
    ratedLoad: safeRatedLoad,
    loadRatio,
  ...
  const getLoadoutAggregateLines = (): string[] => {
    ...
    const {
      ...
      loadRatio,
      weightFactor
    } = calculateWeightFactor(stats.totalWeight, groundRatedLoad)

    const overencumbrance = getOverencumbranceTelemetry(
  stats.totalWeight,
  groundRatedLoad
)
...
const applyMovementArchetypeToPart = (
  ...
  const getGarageEquipValidation = (category: PartCategory, instanceId: string, preview: GarageSnapshot) => {
    ...
    const stats = syncAuthoritativeMechStats()
const groundRatedLoad = getDevPartState('Movement')?.ratedLoad ?? 0
if (predictedWeight > groundRatedLoad ) {
  warnings.push(
    `Ground carry limit exceeded: ${predictedWeight.toFixed(1)} / ${groundRatedLoad .toFixed(1)} kg.`
  )
}
  ...
  const getMovementDebugSnapshot = (): MovementDebugSnapshot => {
    ...
    const movementProfile = getCurrentMovementArchetypeProfile()
      const groundRatedLoad = getDevPartState('Movement')?.ratedLoad ?? 0
    const ratedLoad = Math.max(1, groundRatedLoad )
...
const groundRatedLoad = getDevPartState('Movement')?.ratedLoad ?? 0
    ...
    const overencumbrance = getOverencumbranceTelemetry(stats.totalWeight, groundRatedLoad )
    const currentEnergyRegenPerSecond = getCurrentEnergyRegenPerSecond(stats.totalWeight, groundRatedLoad )
    const staggerResistance = calculateWeightResistance(stats.totalWeight)
    const flightRuntimeProfile = getFlightRuntimeProfile()
...
return [
      ...
      `weight = total:${stats.totalWeight.toFixed(1)} rated:${groundRatedLoad .toFixed(1)} loadRatio:${overencumbrance.loadRatio.toFixed(3)} state:${overencumbrance.state}`,
    ...
    'player.weight.state': {
      ...
      get: () => {
        const stats = syncAuthoritativeMechStats()
          const groundRatedLoad = getDevPartState('Movement')?.ratedLoad ?? 0
        const movementProfile = getCurrentMovementArchetypeProfile()
        return getOverencumbranceTelemetry(stats.totalWeight, groundRatedLoad ).state
      }
    },
  ...
  const executeDeveloperCommand = async (commandLine: string): Promise<string[]> => {
    ...
    if (normalizedCommand.startsWith('player.get ')) {
      ...
      const commandGroundRatedLoad = getDevPartState('Movement')?.ratedLoad ?? 0
      const movementProfile = getCurrentMovementArchetypeProfile()
      const weightTelemetry = getOverencumbranceTelemetry(weight, commandGroundRatedLoad )
      if (mode === 'all') {
        return [
      ...
      if (mode === 'weight') {
        return [
          `weight = ${weight.toFixed(1)}`,
          `ratedLoad = ${groundRatedLoad .toFixed(1)}`,
          `loadRatio = ${weightTelemetry.loadRatio.toFixed(3)}`,
          `loadState = ${weightTelemetry.state}`
        ]
  ...
  const gameLoop = (timestampMs: number): void => {
    ...
    const movementGroundPart = getDevPartState('Movement')
const movementWeightFactor = calculateWeightFactor(effectiveTotalWeight, movementGroundPart .ratedLoad ?? 0).weightFactor
    const flightRuntimeProfile = getFlightRuntimeProfile()
...
const groundMobilityPart = getDevPartState('Movement')
const ratedLoad = groundMobilityPart?.ratedLoad ?? 0

const baseEnergyRegenPerSecond = getCurrentEnergyRegenPerSecond(
